...
(function () {
  ...
  var CACHE_KEY = 'ta_live_v5'; // bumped: invalida cache con date errate pre-2026
  ...
  function getCached(){
    try{
      ...
      if(Date.now()-o.ts<CACHE_TTL){
        // filtra anche dalla cache: scarta notizie precedenti al 2026
        var valid=(o.items||[]).filter(function(i){
          var y=i.data?parseInt(i.data.slice(0,4),10):9999;
          return y>=2026;
        });
  ...
  function getCategory(text){
    ...
    if(/politic|comune|sindac|consiglio|elezioni|partito|voto|assessor/.test(t))return{nome:'Politica',slug:'politica'};
    if(/scuol|sanit|medic|social/.test(t))return{nome:'Società',slug:'societa'};
  ...
  function parseRss2Json(data, sourceName) {
    ...
    try {
      ...
      feed.forEach(function(item) {
        ...
        if (!isNaN(d) && d.getFullYear() < 2026) return; // ignora notizie precedenti al 2026
